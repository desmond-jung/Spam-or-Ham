...
class NaiveBayes():
    ...
    def fit(self, train_hams:list, train_spams:list):
        ...
        def get_counts(filenames:list):
            ...
            for i in range(len(filenames)):
                for word in self.word_set(filenames[i]):
            ...
            return count_dict
        #print(self.word_set(train_hams['had']))
        self.word_counts_ham = get_counts(train_hams)
        self.word_counts_spam = get_counts(train_spams)
        for i in range(len(train_hams)):
            self.num_train_hams += 1
        for i in range(len(train_spams)):
            self.num_train_spams += 1
        #print(self.word_counts_spam)
        #print(self.num_train_hams)
        #print(self.num_train_spams)
        #if(self.word_counts_ham == self.word_counts_spam):
            #print('error')
    ...
    def predict(self, filename:str):
        ...
        3.  an erroFor a dictionary d, d[key] returns the value if the key exists.
        But if it doesn't, you getr. To simplify your code,
        ...
        prob_wham = 0
        #print(self.word_counts_spam)
    #compute probability of email being spam (numerator)
        #print(self.num_train_hams)
        for word in self.word_set(filename):
            #laplace smoothing
            prob_word_spam = np.log10((self.word_counts_spam.get(word,0) + 1)/(self.num_train_spams + 2))
            prob_wspam += prob_word_spam
            #do same for ham
            prob_word_ham = np.log10((self.word_counts_ham.get(word,0) + 1)/(self.num_train_hams + 2))
            prob_wham= prob_wham + prob_word_ham
           
        prob_spam = np.log10(self.num_train_spams/(self.num_train_hams + self.num_train_spams))
        prob_ham = np.log10(self.num_train_hams/(self.num_train_hams + self.num_train_spams))
        spam_prob = prob_spam + prob_wspam
        ham_prob = prob_ham + prob_wham
        #print(ham_prob)
        #print(spam_prob)
        #print(self.num_train_spams)
